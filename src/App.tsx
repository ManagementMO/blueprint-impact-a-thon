...
type Tab = 'home' | 'recommended' | 'week' | 'calendar' | 'circle' | 'support' | 'staff'
...
function App() {
  ...
  const [saved, setSaved] = useState<string[]>(['nature'])
  ...
  const renderHome = () => <section className="screen home-screen">
    ...
    <section className="next-event"><div><p className="eyebrow">COMING UP</p><h2>Saturday morning</h2><p>One calm outdoor event is ready to explore.</p></div><EventCard event={events[0]} onOpen={openEvent} compact slow={slow} /></section><button className="home-about-link" onClick={() => setTab('support')} aria-label="Learn about KW Habilitation"><img src="/kwhab-logo.jpeg" alt="" /><span><strong>About KW Habilitation</strong><small>How we help people live, work, and belong.</small></span><ChevronRight size={22} /></button>
  </section>
